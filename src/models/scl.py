...
class SCL(GeneralRecommender):
    ...
    def calculate_loss(self, interactions):
        ...
        u_1_enc, u_2_enc = self.encoder(u_1), self.encoder(u_2)
        u_1_pred, u_2_pred = self.predictor(u_1_enc), self.predictor(u_2_enc)
        i_1_enc, i_2_enc = self.encoder(i_1), self.encoder(i_2)
        i_1_pred, i_2_pred = self.predictor(i_1_enc), self.predictor(i_2_enc)
        if self.v_feat is not None:
            v_1_enc, v_2_enc = self.encoder(v_1), self.encoder(v_2)
            v_1_pred, v_2_pred = self.predictor(v_1_enc), self.predictor(v_2_enc)
        if self.t_feat is not None:
            t_1_enc, t_2_enc = self.encoder(t_1), self.encoder(t_2)
            t_1_pred, t_2_pred = self.predictor(t_1_enc), self.predictor(t_2_enc)
        ...
        # intra-modal loss
        u_u_loss = self.calculate_distance(u_1_pred, u_2_enc) + self.calculate_distance(u_2_pred, u_1_enc)
        l_intra += u_u_loss
        i_i_loss = self.calculate_distance(i_1_pred, i_2_enc) + self.calculate_distance(i_2_pred, i_1_enc)
        l_intra += i_i_loss
        if self.t_feat is not None:
            t_t_loss = self.calculate_distance(t_1_pred, t_2_enc) + self.calculate_distance(t_2_pred, t_1_enc)
            l_intra += t_t_loss
        if self.v_feat is not None:
            v_v_loss = self.calculate_distance(v_1_pred, v_2_enc) + self.calculate_distance(v_2_pred, v_1_enc)
            l_intra += v_v_loss
        ...
        # inter-modal loss    
        # if self.config['one_predictor']:
        u_enc = self.encoder(u_ori)
        i_enc = self.encoder(i_ori)
        u_pred = self.predictor(u_enc)
        i_pred = self.predictor(i_enc)
        if self.t_feat is not None:
            t_enc = self.encoder(t_feat)
            t_pred = self.predictor(t_enc)
        if self.v_feat is not None:
            v_enc = self.encoder(v_feat)
            v_pred = self.predictor(v_enc)
        # u_pred = self.predictor(self.encoder(u_ori))
        # i_pred = self.predictor(self.encoder(i_ori))
        # if self.t_feat is not None:
        #     t_pred = self.predictor(self.encoder(t_feat))
        # if self.v_feat is not None:
        #     v_pred = self.predictor(self.encoder(v_feat))
        # else:
        ...
        u_i_loss = self.calculate_distance(u_pred, i_enc) + self.calculate_distance(i_pred, u_enc)
        l_inter += u_i_loss
        if self.t_feat is not None:
            i_t_loss = self.calculate_distance(i_pred, t_enc) + self.calculate_distance(t_pred, i_enc)
            l_inter += i_t_loss
        if self.v_feat is not None:
            i_v_loss = self.calculate_distance(i_pred, v_enc) + self.calculate_distance(v_pred, i_enc)
            l_inter += i_v_loss
        if self.t_feat is not None and self.v_feat is not None:
            t_v_loss = self.calculate_distance(t_pred, v_enc) + self.calculate_distance(v_pred, t_enc)
    ...
    def full_sort_predict(self, interaction):
        ...
        if self.config['use_predictor_final']:
            u_online, i_online = self.predictor(self.encoder(u_online)), self.predictor(self.encoder(i_online))
        else:
            u_online, i_online = self.encoder(u_online), self.encoder(i_online)
